...
- **`AMBAdvisorCompetencesDTO` is now a Callable class.**
    - The `AMBAdvisorCompetencesDTO` class was previously a private class in the managed package which made it impossible to use outside the &schedule-package.
...
- Breaking change:
    ...
    - The new names are `scheduleEmployeeFlow` and `scheduleCustomerFlow`
...
-  Added translations for schedule customLabels for numerous languages (The used language will depend on the specific user's language setting in the organization).
