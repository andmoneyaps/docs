...
When an advisor books an out-of-office meeting ("ude af huset"), they type part of an address and pick it from a suggestion list. Schedule uses the chosen start and end addresses to calculate travel time and to add travel slots to the advisor's calendar.
...
Adressevælgeren replaces DAWA (Danmarks Adressers Web API), the provider Schedule used before. Klimadatastyrelsen retires DAWA on 1 October 2026.

The lookup runs in the advisor's browser: a search that returns matching addresses, then a lookup of the chosen address that returns its position. No Schedule configuration is needed. Your organisation may need to allow the host, see below.
...
- **No credentials to manage.** Schedule accesses Adressevælgeren with a shared access key that Klimadatastyrelsen prescribes for the transition period. Klimadatastyrelsen plans per-organisation access management later (expected late 2026 or early 2027). Schedule will absorb that change; no action is needed from you now.
