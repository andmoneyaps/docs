...
# Assist - Meeting Goals

---

## Overview

Assist - Meeting Goals gives the Admin/configurators access to define the objectives that an employee should reach during a meeting. Each meeting goal is tied to a **Meeting Topic** (and optionally a **Meeting Subtopic**). The meeting's Topic is fixed when the meeting is booked, so the matching meeting goals load automatically when the employee opens the meeting — there is nothing for the employee to pick. Subtopics can inherit meeting goal from their parent topic by default, and each inherited goal can optionally be excluded on a per-subtopic basis. During the meeting, Assist's AI uses each goal's **AI Instruction** to judge in real time whether the goal has been reached, surfacing this back to the employee through the live insights and the post-meeting summary.
...
1. Open the &money Management UI
2. Navigate to **Assist** → **Meeting Goals** → **Meeting Goals**

The page lists every meeting goal grouped by the topic it belongs to.
...
|---------|------------|----------|
| **Owned** (`{Topic name}`) | Viewing all topics, a single topic, or a subtopic | Meeting goals defined directly on that topic or subtopic. Includes a **Create** button. |
| **Inherited from {Topic}** | A subtopic is selected | Meeting goals inherited from the parent topic. Each row has an **Include** toggle — on by default — that can be flipped off to exclude that single goal from the selected subtopic. |
| **Orphaned goals** | Orphans exist and the `Orphans` filter is selected | Meeting goals whose original topic has been deleted. |

### Meeting Goal Table Columns
...
## Creating a Meeting Goal

1. Open the section for the topic or subtopic you want the meeting goal to belong to
2. Click **Create goal**
3. Fill in the form:
...
| **Subtopic** | No | Leave empty to apply the meeting goal to *all* subtopics of the selected topic. Selecting a subtopic restricts the meeting goal to only that subtopic. |
| **Name** | Yes | Must be unique within the topic hierarchy. |
| **Description** | Yes | Max 500 characters. Not shown to employees in the meeting UI, but fed to the AI together with the **Name** as the source material for the auto-generated AI Instruction (**Create instructions**). |
| **AI Instruction** | Yes | The instruction Assist's AI uses to judge whether the meeting goal has been reached. |
...
When a goal is created on a topic that has subtopics (i.e. **Subtopic** is left empty), a confirmation dialog lists every subtopic the meeting goal will be inherited by. Tick the acknowledgement checkbox to continue.

> **Tip**: To restrict a meeting goal to a single subtopic, select that subtopic in the **Subtopic** field. To restrict a goal to a few subtopics, create it on the parent topic and then exclude it on the subtopics where it should not apply (see [Excluding Inherited Goals](#excluding-inherited-goals)).
...
## Editing a Meeting Goal

1. Click the **Edit** icon on the meeting goal's row
...
Moving a Meeting Goal to a different topic by changing the **Topic** field will, on save, re-evaluate inheritance for the new owner topic and its subtopics.

---

## Excluding Inherited Goals

By default, every meeting goal defined on a topic is inherited by all of its subtopics. Each inherited goal can optionally be excluded from any individual subtopic — the rest of the inheritance stays intact.
...
As soon as the employee enables **Meeting Goals** in the settings, Assist resolves the applicable goal list for the meeting's booked Topic — every goal defined directly on that Topic (or its Subtopic, if one was booked), plus every inherited goal from the parent Topic that has *not* been excluded for the booked Subtopic. The result is shown in a **Your goals for: {Topic name}** preview card so the employee can see what will be tracked before the meeting starts.
...
Every Meeting Goal moves through three states. The AI assigns the state based on what it hears in the transcript, and the assignment is **forward-only** — a goal that has reached *Addressed* will never drop back to *Not started*, and a *Completed* goal will never regress.
...
Assist evaluates goals in the background while the meeting runs. The advisor does not need to trigger anything.
...
- **Per-goal prompt** — For each goal, the AI is given the goal's **Name**, **Description**, and the **AI Instruction** configured in the Management UI. The AI Instruction is the authoritative rule for when the goal counts as reached. (If a goal's AI Instruction is **Missing**, Assist falls back to using its Description — the goal is still evaluated but with materially lower quality.)
...
- **Goals without an AI Instruction.** Assist falls back to the Description in place of the AI Instruction. The goal still appears in the tracker and is still evaluated, but accuracy is materially lower — always configure an explicit AI Instruction before going to production.

---

## Reporting

The final goal states recorded during each meeting roll up into a dashboard so configurators and team leads can see how the goal configuration is performing across the organisation. There is nothing extra to enable — every Assist session with goals turned on automatically contributes to this dashboard.

### Accessing the dashboard

1. Open the &money Management UI
2. Navigate to **Assist** → **Reporting** → **Reporting**
...
| Tile | Meaning |
|------|---------|
| **Assist meetings** | Total number of Assist sessions in the period (whether or not goals were enabled). The baseline everything else is measured against. |
| **Meetings with goals** | Share of those sessions where the employee turned **Meeting Goals** on, shown as a percentage with the underlying numerator/denominator (e.g. *2 of 37 meetings*). A low number here usually means goals weren't enabled, not that goals failed. |
| **Goal count** | Total number of Meeting Goal *evaluations* across every goal-tracked meeting in the period. A single meeting with 5 goals contributes 5 to this number. |
