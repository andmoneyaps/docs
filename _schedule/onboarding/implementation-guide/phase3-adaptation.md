...
redirect_from:
  - /bookme/onboarding/implementation-guide/phase3-adaptation/
---

# Phase 3 - Adaptation & Development

This phase covers the technical implementation and customization of &schedule Scheduler.
...
&schedule - Scheduler offers a wide range of configurations/customizations that ensure the product fits into your sales pipeline. In this section you will find the documentation for the fixes, configurations and customizations &schedule - Scheduler supports in relation to the implementation of the solution in complex organizations.
...
![Sales pipeline]({{ site.baseurl }}/assets/images/schedule/onboarding/implementation-guide/sales-pipeline.png)
...
In &schedule - Scheduler, 3 different location concepts are used.
...
The **advisor's location** is given to &schedule - Scheduler through Azure SCIM. If this location exactly matches the text string displayed as the **customer's location**, the advisor will be able to appear under "department" in the advisor flow.

The **meeting room location** is given to &schedule - Scheduler through Azure SCIM, and just like the **advisor's location**, this is given via Azure SCIM. The list of meeting rooms in the advisor flow will therefore always be filtered down to meeting rooms whose location matches the customer's location.
...
## &Schedule – setup requirements and points to consider
...
#### **Points of attention**
- &schedule - Scheduler synchronizes calendars up to n days into the future (default 60 days). This means that a meeting booked further into the future than n days will only take into account free time for other meetings booked through &schedule
- Repeated meetings and meetings over several days (e.g. vacation) that were booked before &schedule synchronization was enabled are currently not synchronized
- Past meetings cannot be edited via &schedule - Scheduler
...
- For troubleshooting in Salesforce, we often need to enable the debug log and then send the log to &schedule's service desk.
- In case of errors on booked meetings, it is important to include the meeting's bookingId for troubleshooting in the &schedule -Scheduler solution
...
To use the feature, the advisor must book the &schedule - Scheduler Integration User in their outlook calendar (the user that the bank has set up for the MS365 integration service - e.g. the bank's IT department has the details of this user).
This signals to &schedule - Scheduler that the advisor is available outside of normal availability, but only for online and telephone meetings.

![Extraordinary calendar availability]({{ site.baseurl }}/assets/images/schedule/onboarding/implementation-guide/calendar-availability.png)
