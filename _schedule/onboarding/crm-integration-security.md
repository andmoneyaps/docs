...
redirect_from:
  - /bookme/onboarding/crm-integration-security/
---
...
For the Schedule CRM Integration to work with an organization's CRM platform, it must be given the required access to   
...
- When a meeting is booked through the Partner portal, the integration will create a new event in the CRM platform and the related sales pipeline object (Sales Lead) together with event relations and internal Schedule records.


## Salesforce access requirements (Org level security)

For the integration to work, the integration must have access via the org-level security layer. This access does not deviate from the standard Schedule integration model.


| Requirement      | Description                                                                                                                                                                                 |
|------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| IP restrictions  | The integration must be able to access the CRM platform via the API. This means that the IP addresses of the Schedule platform must be whitelisted in the CRM platform.                       |
...
You as the customer then use the Management UI to map the data from the Schedule platform to the CRM platform and describe which sObjects to create and relate.
...
The Standard permission set ensures that the Integration User has the necessary permissions to create the standard Schedule custom objects in the CRM platform and custom customer relations.
